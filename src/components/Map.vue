<template>
  <div id="map" ref="map">
    ...
    <div ref="twitterPopup" class="ol-popup ol-twitterpopup">
      <div
        ref="twitterpopupCloser"
        class="ol-popup-closer"
        v-on:click="closePopup"
      ></div>
      <div class="ol-popup-twitter-content">
        <div ref="loadingTweets" class="twitter-loading hidden">
          Loading Tweets...
        </div>
...
export default {
  ...
  created: function () {
    this.$eventBus.on("set-map-view", this.setMapView);
  },
  beforeDestroy: function () {
    this.$eventBus.off("set-map-view", this.setMapView);
  },
  computed: {
  ...
  methods: {
        ...
        this.olmap.on("singleclick", (e) => {
          ...
          if (feature) {
            ...
            if (props.title && props.image && props.text1) {
              ...
              this.closeTooltip();
            } else if (props.key2 && props.date && props.City_County) {
              this.$refs.popupContent.innerHTML = props.key2
                ? `<strong>${props.key2}</strong>`
                : "";
              this.$refs.popupContent.innerHTML += props.River
                ? `<br>${props.River}<br>`
                : "";
              this.$refs.popupContent.innerHTML += props.date
                ? ` * * * *<br>Takedown date: ${props.date}<br>`
                : "";
              this.$refs.popupContent.innerHTML += props.Dam_Height_ft
                ? `Dam height in feet: ${props.Dam_Height_ft}<br>`
                : "";
              this.$refs.popupContent.innerHTML += props.Year_Built
                ? `Year built: ${props.Year_Built}<br>`
                : "";
              this.$refs.popupContent.innerHTML += props.Original_Use
                ? `Original use: ${props.Original_Use}<br>`
                : "";
              this.$refs.popupContent.innerHTML += props.Type_Material
                ? `Type of material: ${props.Type_Material}`
                : "";
              this.popup.setPosition(e.coordinate);
            } else if (props.key2) {
              this.$refs.popupContent.innerHTML = props.key2
                ? `<strong>${props.key2}</strong><br>`
                : "";
              this.$refs.popupContent.innerHTML += props.date
                ? `<br>Built: ${props.date}`
                : "";
              this.$refs.popupContent.innerHTML += props.Date_idled
                ? `<br>Idled: ${props.Date_idled}`
                : "";
              this.$refs.popupContent.innerHTML += props.NID_HEIGHT
                ? `<br>Height: ${props.NID_HEIGHT} ft`
                : "";
              this.$refs.popupContent.innerHTML += props.wiki_entry
                ? `<br>Wikipedia entry <a href='https://en.wikipedia.org/wiki/${props.wiki_entry}' target='_blank'>here</a>`
                : "";
              this.$refs.popupContent.innerHTML += props.Cleanup_progress
    ...
    geoJSONPointVectorLayerStyle: function (feature) {
      ...
      if (
        feature.values_ &&
        feature.values_["icon"] &&
        !this.styleCache[feature.values_["icon"]]
      ) {
    ...
    makeGeoJSONPointVectorLayerWithStyle: function (
      url,
      label,
      minResolution,
      maxResolution,
